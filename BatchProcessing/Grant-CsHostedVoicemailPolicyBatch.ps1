...
<#
    .SYNOPSIS
    Grants the given HostedVoicemailPolicy
    
    .DESCRIPTION
    Grants the given HostedVoicemailPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsHostedVoicemailPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsHostedVoicemailPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsHostedVoicemailPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsHostedVoicemailPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsHostedVoicemailPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsHostedVoicemailPolicy @ArgHash
'@)

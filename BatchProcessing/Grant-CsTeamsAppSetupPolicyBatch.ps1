...
<#
    .SYNOPSIS
    Grants the given TeamsAppSetupPolicy
    
    .DESCRIPTION
    Grants the given TeamsAppSetupPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsAppSetupPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsAppSetupPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsAppSetupPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsAppSetupPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsAppSetupPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsAppSetupPolicy @ArgHash
'@)

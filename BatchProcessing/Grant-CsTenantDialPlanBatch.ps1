...
<#
    .SYNOPSIS
    Grants the given TenantDialPlan
    
    .DESCRIPTION
    Grants the given TenantDialPlan for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTenantDialPlanBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTenantDialPlanBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTenantDialPlanBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTenantDialPlanBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTenantDialPlanBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTenantDialPlan @ArgHash
'@)

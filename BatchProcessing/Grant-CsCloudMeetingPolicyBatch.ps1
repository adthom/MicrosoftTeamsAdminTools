...
<#
    .SYNOPSIS
    Grants the given CloudMeetingPolicy
    
    .DESCRIPTION
    Grants the given CloudMeetingPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsCloudMeetingPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsCloudMeetingPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsCloudMeetingPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsCloudMeetingPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsCloudMeetingPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsCloudMeetingPolicy @ArgHash
'@)

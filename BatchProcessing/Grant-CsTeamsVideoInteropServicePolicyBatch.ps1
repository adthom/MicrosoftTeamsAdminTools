...
<#
    .SYNOPSIS
    Grants the given TeamsVideoInteropServicePolicy
    
    .DESCRIPTION
    Grants the given TeamsVideoInteropServicePolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsVideoInteropServicePolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsVideoInteropServicePolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsVideoInteropServicePolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsVideoInteropServicePolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsVideoInteropServicePolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsVideoInteropServicePolicy @ArgHash
'@)

...
<#
    .SYNOPSIS
    Grants the given TeamsMeetingPolicy
    
    .DESCRIPTION
    Grants the given TeamsMeetingPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsMeetingPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsMeetingPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsMeetingPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsMeetingPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsMeetingPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsMeetingPolicy @ArgHash
'@)

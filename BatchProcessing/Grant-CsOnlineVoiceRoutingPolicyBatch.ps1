...
<#
    .SYNOPSIS
    Grants the given OnlineVoiceRoutingPolicy
    
    .DESCRIPTION
    Grants the given OnlineVoiceRoutingPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsOnlineVoiceRoutingPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsOnlineVoiceRoutingPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsOnlineVoiceRoutingPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsOnlineVoiceRoutingPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsOnlineVoiceRoutingPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsOnlineVoiceRoutingPolicy @ArgHash
'@)

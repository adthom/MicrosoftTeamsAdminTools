...
<#
    .SYNOPSIS
    Grants the given TeamsMeetingBroadcastPolicy
    
    .DESCRIPTION
    Grants the given TeamsMeetingBroadcastPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsMeetingBroadcastPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsMeetingBroadcastPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsMeetingBroadcastPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsMeetingBroadcastPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsMeetingBroadcastPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsMeetingBroadcastPolicy @ArgHash
'@)

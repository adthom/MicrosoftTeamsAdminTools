...
<#
    .SYNOPSIS
    Grants the given ApplicationAccessPolicy
    
    .DESCRIPTION
    Grants the given ApplicationAccessPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsApplicationAccessPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsApplicationAccessPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsApplicationAccessPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsApplicationAccessPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsApplicationAccessPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsApplicationAccessPolicy @ArgHash
'@)

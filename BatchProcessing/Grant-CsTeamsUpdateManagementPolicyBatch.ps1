...
<#
    .SYNOPSIS
    Grants the given TeamsUpdateManagementPolicy
    
    .DESCRIPTION
    Grants the given TeamsUpdateManagementPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsUpdateManagementPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsUpdateManagementPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsUpdateManagementPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsUpdateManagementPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsUpdateManagementPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsUpdateManagementPolicy @ArgHash
'@)

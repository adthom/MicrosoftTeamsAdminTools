...
<#
    ...
    .EXAMPLE
    Invoke-CsOrgMeetingMigration -UserName admin1@contoso.com, admin2@contoso.com -IsTest $false
    This runs a migration for the full tenant leveraging 2 administrator accounts for a maximum of 6 concurrent sessions

    .EXAMPLE
    Invoke-CsOrgMeetingMigration -UserName admin1@contoso.com -UsersFilePath .\userstomigrate.txt
    This runs a test migration for the users listed in .\userstomigrate.txt using 1 administrator account
#>

param(
    # Specifies if Start-CsExMeetingMigration should be run with WhatIf switch, default is $true, set to $false to actually perform the migration
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsOrgMeetingMigration_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Invoke-CsOrgMeetingMigration"
...
param($filterSB, $IsTest)
Get-CsOnlineUser -Filter $filterSB | 
Start-CsExMeetingMigration -SourceMeetingType SfB -TargetMeetingType Teams -Confirm:$false -WhatIf:$IsTest 
'@)

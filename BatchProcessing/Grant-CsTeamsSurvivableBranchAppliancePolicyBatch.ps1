...
<#
    .SYNOPSIS
    Grants the given TeamsSurvivableBranchAppliancePolicy
    
    .DESCRIPTION
    Grants the given TeamsSurvivableBranchAppliancePolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsSurvivableBranchAppliancePolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsSurvivableBranchAppliancePolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsSurvivableBranchAppliancePolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsSurvivableBranchAppliancePolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsSurvivableBranchAppliancePolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsSurvivableBranchAppliancePolicy @ArgHash
'@)

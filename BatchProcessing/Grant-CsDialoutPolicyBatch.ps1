...
<#
    .SYNOPSIS
    Grants the given DialoutPolicy
    
    .DESCRIPTION
    Grants the given DialoutPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsDialoutPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsDialoutPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsDialoutPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsDialoutPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsDialoutPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsDialoutPolicy @ArgHash
'@)

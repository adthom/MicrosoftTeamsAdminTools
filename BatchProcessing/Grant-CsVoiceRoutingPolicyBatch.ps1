...
<#
    .SYNOPSIS
    Grants the given VoiceRoutingPolicy
    
    .DESCRIPTION
    Grants the given VoiceRoutingPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsVoiceRoutingPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsVoiceRoutingPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsVoiceRoutingPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsVoiceRoutingPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsVoiceRoutingPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsVoiceRoutingPolicy @ArgHash
'@)

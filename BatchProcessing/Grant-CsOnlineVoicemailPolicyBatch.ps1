...
<#
    .SYNOPSIS
    Grants the given OnlineVoicemailPolicy
    
    .DESCRIPTION
    Grants the given OnlineVoicemailPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsOnlineVoicemailPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsOnlineVoicemailPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsOnlineVoicemailPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsOnlineVoicemailPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsOnlineVoicemailPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsOnlineVoicemailPolicy @ArgHash
'@)

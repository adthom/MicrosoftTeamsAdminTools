...
<#
    .SYNOPSIS
    Grants the given TeamsChannelsPolicy
    
    .DESCRIPTION
    Grants the given TeamsChannelsPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsChannelsPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsChannelsPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsChannelsPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsChannelsPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsChannelsPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsChannelsPolicy @ArgHash
'@)

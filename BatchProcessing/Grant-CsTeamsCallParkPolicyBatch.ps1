...
<#
    .SYNOPSIS
    Grants the given TeamsCallParkPolicy
    
    .DESCRIPTION
    Grants the given TeamsCallParkPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsCallParkPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsCallParkPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsCallParkPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsCallParkPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsCallParkPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsCallParkPolicy @ArgHash
'@)

...
<#
    .SYNOPSIS
    Grants the given TeamsUpgradePolicy
    
    .DESCRIPTION
    Grants the given TeamsUpgradePolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsUpgradePolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName UpgradeToTeams -MigrateMeetingsToTeams $true -IsTest $false
    This runs a migration for all enabled users in the tenant leveraging 2 administrator accounts for a maximum of 6 concurrent sessions

    .EXAMPLE
    Grant-CsTeamsUpgradePolicyBatch -UserName admin1@contoso.com -PolicyName UpgradeToTeams -MigrateMeetingsToTeams $true -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies whether to move existing Skype for Business meetings organized by the user to Teams. This parameter can only be true if the mode of the specified policy instance is either TeamsOnly or SfBWithTeamsCollabAndMeetings
    [bool]$MigrateMeetingsToTeams,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsUpgradePolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsUpgradePolicyBatch"

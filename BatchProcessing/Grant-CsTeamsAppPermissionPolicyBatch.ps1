...
<#
    .SYNOPSIS
    Grants the given TeamsAppPermissionPolicy
    
    .DESCRIPTION
    Grants the given TeamsAppPermissionPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsAppPermissionPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsAppPermissionPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsAppPermissionPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsAppPermissionPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsAppPermissionPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsAppPermissionPolicy @ArgHash
'@)

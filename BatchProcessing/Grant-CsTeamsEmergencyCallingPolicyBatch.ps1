...
<#
    .SYNOPSIS
    Grants the given TeamsEmergencyCallingPolicy
    
    .DESCRIPTION
    Grants the given TeamsEmergencyCallingPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsEmergencyCallingPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsEmergencyCallingPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsEmergencyCallingPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsEmergencyCallingPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsEmergencyCallingPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsEmergencyCallingPolicy @ArgHash
'@)

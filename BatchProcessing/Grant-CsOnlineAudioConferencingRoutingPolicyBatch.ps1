...
<#
    .SYNOPSIS
    Grants the given OnlineAudioConferencingRoutingPolicy
    
    .DESCRIPTION
    Grants the given OnlineAudioConferencingRoutingPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsOnlineAudioConferencingRoutingPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsOnlineAudioConferencingRoutingPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsOnlineAudioConferencingRoutingPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsOnlineAudioConferencingRoutingPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsOnlineAudioConferencingRoutingPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsOnlineAudioConferencingRoutingPolicy @ArgHash
'@)

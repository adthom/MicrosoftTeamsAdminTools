...
<#
    .SYNOPSIS
    Grants the given TeamsVdiPolicy
    
    .DESCRIPTION
    Grants the given TeamsVdiPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsVdiPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsVdiPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsVdiPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsVdiPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsVdiPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsVdiPolicy @ArgHash
'@)

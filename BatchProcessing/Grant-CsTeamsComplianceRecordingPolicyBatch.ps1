...
<#
    .SYNOPSIS
    Grants the given TeamsComplianceRecordingPolicy
    
    .DESCRIPTION
    Grants the given TeamsComplianceRecordingPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsComplianceRecordingPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsComplianceRecordingPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsComplianceRecordingPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsComplianceRecordingPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsComplianceRecordingPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsComplianceRecordingPolicy @ArgHash
'@)

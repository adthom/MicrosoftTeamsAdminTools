...
<#
    .SYNOPSIS
    Grants the given TeamsIPPhonePolicy
    
    .DESCRIPTION
    Grants the given TeamsIPPhonePolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsIPPhonePolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsIPPhonePolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsIPPhonePolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsIPPhonePolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsIPPhonePolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsIPPhonePolicy @ArgHash
'@)

...
<#
    .SYNOPSIS
    Grants the given MobilityPolicy
    
    .DESCRIPTION
    Grants the given MobilityPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsMobilityPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsMobilityPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsMobilityPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsMobilityPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsMobilityPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsMobilityPolicy @ArgHash
'@)

...
<#
    .SYNOPSIS
    Grants the given TeamsCortanaPolicy
    
    .DESCRIPTION
    Grants the given TeamsCortanaPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsCortanaPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsCortanaPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsCortanaPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsCortanaPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsCortanaPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsCortanaPolicy @ArgHash
'@)

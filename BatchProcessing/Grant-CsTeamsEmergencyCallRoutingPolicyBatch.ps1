...
<#
    .SYNOPSIS
    Grants the given TeamsEmergencyCallRoutingPolicy
    
    .DESCRIPTION
    Grants the given TeamsEmergencyCallRoutingPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsEmergencyCallRoutingPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsEmergencyCallRoutingPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsEmergencyCallRoutingPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsEmergencyCallRoutingPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsEmergencyCallRoutingPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsEmergencyCallRoutingPolicy @ArgHash
'@)

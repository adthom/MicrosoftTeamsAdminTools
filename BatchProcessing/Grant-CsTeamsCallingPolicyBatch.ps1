...
<#
    .SYNOPSIS
    Grants the given TeamsCallingPolicy
    
    .DESCRIPTION
    Grants the given TeamsCallingPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsCallingPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsCallingPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsCallingPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsCallingPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsCallingPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsCallingPolicy @ArgHash
'@)

...
<#
    .SYNOPSIS
    Grants the given TeamsVerticalPackagePolicy
    
    .DESCRIPTION
    Grants the given TeamsVerticalPackagePolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsVerticalPackagePolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsVerticalPackagePolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsVerticalPackagePolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsVerticalPackagePolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsVerticalPackagePolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsVerticalPackagePolicy @ArgHash
'@)

...
<#
    .SYNOPSIS
    Grants the given ClientPolicy
    
    .DESCRIPTION
    Grants the given ClientPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsClientPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsClientPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsClientPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsClientPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsClientPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsClientPolicy @ArgHash
'@)

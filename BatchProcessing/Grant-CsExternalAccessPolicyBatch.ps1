...
<#
    .SYNOPSIS
    Grants the given ExternalAccessPolicy
    
    .DESCRIPTION
    Grants the given ExternalAccessPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsExternalAccessPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsExternalAccessPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsExternalAccessPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsExternalAccessPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsExternalAccessPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsExternalAccessPolicy @ArgHash
'@)

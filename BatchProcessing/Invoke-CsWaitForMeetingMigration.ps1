...
<#
    .SYNOPSIS
    Gets Meeting Migration Status for a large tenant and waits for completion
    
    .DESCRIPTION
    Gets Meeting Migration Status for all users enabled in a Tenant or from a user provided list and will wait for migration completion
    ...
    .OUTPUTS
    Outputs PSCustomObject of waited users with their migration statuses from the searched period

    .EXAMPLE
    Invoke-CsWaitForMeetingMigration -UserName admin1@contoso.com, admin2@contoso.com
    This gets status for the full tenant leveraging 2 administrator accounts for a maximum of 6 concurrent sessions

    .EXAMPLE
    Invoke-CsWaitForMeetingMigration -UserName admin1@contoso.com -UsersFilePath .\userstomigrate.txt
    This gets status for the users listed in .\userstomigrate.txt using 1 administrator account
#>
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsWaitForMeetingMigration_$LogDate.log"

$FilterScript = [ScriptBlock]::Create('Enabled -eq $true')
$JobScript = [ScriptBlock]::Create(@'
param($filterSB, $StartTime, $EndTime)
Get-CsOnlineUser -Filter $filterSB | 
Get-CsMeetingMigrationStatus -StartTime $StartTime -EndTime $EndTime
'@)

Write-Log -Level Info -Path $LogFile -Message "Invoke-CsWaitForMeetingMigration"
...
do {
    Write-Log -Level Info -Path $LogFile -Message "Getting migration status..."
    Write-Host "Getting migration status..."
    $Results = @(Invoke-CsOnlineBatch @BatchParams)

    $RunningMigrations = @($Results.Where( { $_.State -eq "InProgress" }))
    $PendingMigrations = @($Results.Where( { $_.State -eq "Pending" }))
    $FailedMigrations = @($Results.Where( { $_.State -eq "Failed" }))
    $SuccessfulMigrations = @($Results.Where( { $_.State -eq "Succeeded" }))

    $UnFinishedMigrations = $RunningMigrations + $PendingMigrations

    Write-Log -Level Info -Path $LogFile -Message "Number of objects returned from batch: $($Results.Count)"

    if ($UnFinishedMigrations.Count -gt 0) {
        $SleepSeconds = $SleepMinutes * 60
        Write-Log -Level Info -Path $LogFile -Message "Migrations actively running:`t$($RunningMigrations.Count)"
        Write-Host "Migrations actively running:`t$($RunningMigrations.Count)"
        Write-Log -Level Info -Path $LogFile -Message "Migrations still pending:`t$($PendingMigrations.Count)"
        Write-Host "Migrations still pending:`t$($PendingMigrations.Count)"

        if ($PendingMigrations.Count -le 5) {
            Write-Host "Users:`t$($PendingMigrations.UserPrincipalName -join ', ')"
        }

        $OldestMigration = $PendingMigrations | Sort-Object -Property CreateDate | Select-Object -First 1
        Write-Log -Level Info -Path $LogFile -Message "Oldest Pending Migration Created:`t$($OldestMigration.CreateDate)"
        Write-Host "Oldest Pending Migration Created:`t$($OldestMigration.CreateDate)"

        Write-Log -Level Info -Path $LogFile -Message "Migrations in Failed State:`t$($FailedMigrations.Count)"
        Write-Host "Migrations in Failed State:`t$($FailedMigrations.Count)"
        Write-Log -Level Info -Path $LogFile -Message "Migrations in Successful State:`t$($SuccessfulMigrations.Count)"
        Write-Host "Migrations in Successful State:`t$($SuccessfulMigrations.Count)"

        Write-Log -Level Info -Path $LogFile -Message "Sleeping $SleepSeconds seconds..."
        Write-Host "Sleeping $SleepSeconds seconds..."
        Write-Host ""
        Start-Sleep -Seconds $SleepSeconds
    }
    else {
        Write-Log -Level Info -Path $LogFile -Message "Migrations finished!"
        ...
        Write-Host "Migrations in Successful State:`t$($SuccessfulMigrations.Count)"
        $ExcludedProps = @(
            'PSComputerName', 'PSShowComputerName', 'RunspaceId', 'UserId', 'State', 'CorrelationId', 'LastMessage'
        )
        $FailedMigrations | Select-Object -ExcludeProperty $ExcludedProps -Property *, @{Name='Success'; Expression={$false}}
        $SuccessfulMigrations | Select-Object -ExcludeProperty $ExcludedProps -Property *, @{Name='Success'; Expression={$true}}
    }
} while ($UnFinishedMigrations.Count -gt 0)

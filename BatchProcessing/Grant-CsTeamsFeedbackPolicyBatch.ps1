...
<#
    .SYNOPSIS
    Grants the given TeamsFeedbackPolicy
    
    .DESCRIPTION
    Grants the given TeamsFeedbackPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsFeedbackPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsFeedbackPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsFeedbackPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsFeedbackPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsFeedbackPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsFeedbackPolicy @ArgHash
'@)

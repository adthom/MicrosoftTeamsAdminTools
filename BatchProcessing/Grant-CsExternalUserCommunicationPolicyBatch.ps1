...
<#
    .SYNOPSIS
    Grants the given ExternalUserCommunicationPolicy
    
    .DESCRIPTION
    Grants the given ExternalUserCommunicationPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsExternalUserCommunicationPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsExternalUserCommunicationPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsExternalUserCommunicationPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsExternalUserCommunicationPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsExternalUserCommunicationPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsExternalUserCommunicationPolicy @ArgHash
'@)

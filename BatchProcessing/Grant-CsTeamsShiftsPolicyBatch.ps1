...
<#
    .SYNOPSIS
    Grants the given TeamsShiftsPolicy
    
    .DESCRIPTION
    Grants the given TeamsShiftsPolicy for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsTeamsShiftsPolicyBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsTeamsShiftsPolicyBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsTeamsShiftsPolicyBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsTeamsShiftsPolicyBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsTeamsShiftsPolicyBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsTeamsShiftsPolicy @ArgHash
'@)

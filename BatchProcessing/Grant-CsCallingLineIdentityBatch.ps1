...
<#
    .SYNOPSIS
    Grants the given CallingLineIdentity
    
    .DESCRIPTION
    Grants the given CallingLineIdentity for all users enabled or from a user provided list.
    ...
    .EXAMPLE
    Grant-CsCallingLineIdentityBatch -UserName admin1@contoso.com, admin2@contoso.com -PolicyName $null -IsTest $false
    ...
    .EXAMPLE
    Grant-CsCallingLineIdentityBatch -UserName admin1@contoso.com -PolicyName $null -UsersFilePath .\userstomigrate.txt
...
param(
    ...
    # Specifies if Grant-CsCallingLineIdentityBatch should be run with WhatIf switch, default is $true, set to $false to actually perform the change
    [bool]$IsTest = $true,
...
$LogFolderPath = Resolve-Path $LogFolderPath
$LogDate = Get-Date -Format "yyyyMMdd_HHmmss"
$LogFile = "$LogFolderPath\CsCallingLineIdentityBatch_$LogDate.log"

Write-Log -Level Info -Path $LogFile -Message "Grant-CsCallingLineIdentityBatch"
...
Get-CsOnlineUser -Filter $filterSB | Grant-CsCallingLineIdentity @ArgHash
'@)
